...
NOTE: More information about the components can be found in xref:developers/architecture.adoc[] chapter.
...
|`hawtio.disableProxy`
|`false` in SpringBoot and WAR deployments, `true` in Quarkus deployments.
...
|`true`
|Whether https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/net/NetworkInterface.html#getNetworkInterfaces()[all local addresses] for proxy allowlist should be allowed. Set this property to `false` to use only `127.0.0.1` and `localhost` addresses.
...
|`hawtio.proxyDisableCertificateValidation`
|`false`
a|Whether to disable hostname verifier in HttpClient4 used by `ProxyServlet` when using TLS connections. +

This option can also be specified using `PROXY_DISABLE_CERT_VALIDATION` environment variable.

|`hawtio.redirect.scheme`
|-
|The scheme of the redirect URL to login page when authentication is required. When this scheme is not configured, Hawtio sends redirects in the form
of `/hawtio/login` instead of absolute address with host name and port number.
...
|`hawtio.http.enableCORS`
|`false`
|Whether CORS filter is enabled and checks for permitted `Origin` HTTP header values.

|`hawtio.http.accessControlAllowOrigin`
|`*`
|When `hawtio.http.enableCORS` option is enabled, Hawtio responds to https://developer.mozilla.org/en-US/docs/Glossary/Preflight_request[CORS pre-flight requests] with CORS headers. This options allows to set the value returned in `Access-Control-Allow-Origin` response header.

|`hawtio.http.allowXFrameSameOrigin`
|`false`
a|When set to `true`, Hawtio sends response headers:
...
|`strict-origin`
|What value Hawtio sends with https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Referrer-Policy[`Referrer-Policy`] response header.

|`hawtio.http.csp.connectSrc`
|-
|(Since Hawtio 5.1.0) Source list of custom `connect-src` directive that users can add to the default https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy[`Content-Security-Policy`] response header sent by Hawtio. Specify a comma-separated list of allowed sources to connect if a specific plugin requires access to external resources.

|`hawtio.http.csp.scriptSrc`
|-
|(Since Hawtio 5.1.0) Source list of custom `script-src` directive that users can add to the default https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy[`Content-Security-Policy`] response header sent by Hawtio. Specify a comma-separated list of allowed sources for scripting if a specific plugin requires loading external scripts.

|`hawtio.http.csp.scriptSrcElem`
|-
|(Since Hawtio 5.1.0) Source list of custom `script-src-elem` directive that users can add to the default https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy[`Content-Security-Policy`] response header sent by Hawtio. Specify a comma-separated list of allowed sources for scripting if a specific plugin requires loading external scripts.
|===
...
Since Jolokia 2.2.0 all Jolokia properties can be specified as `jolokia.` prefixed system properties or `JOLOKIA_` prefixed
environment variables. See https://jolokia.org/reference/html/manual/agents.html#_configuration[Jolokia Configuration].
...
----
{
  "branding": {
    ...
    "appLogoDarkModeUrl": "hawtio-logo-darkmode.svg",
    "css": "",
    "favicon": "favicon.ico"
  },
  "login": {
  ...
  "about": {
    ...
    "imgDarkModeSrc": "hawtio-logo-darkmode.svg",
    "productInfo": [
...
|`appLogoUrl` |`img/hawtio-logo.svg` |Use the URL to substitute the application logo to be used in light mode.
|`appLogoDarkModeUrl` |`img/hawtio-logo.svg` |Use the URL to substitute the application logo to be used in dark mode.
...
|`imgSrc` |`img/hawtio-logo.svg` |Use the URL to substitute the logo image in the About modal to be used in light mode.
|`imgDarkModeSrc` |`img/hawtio-logo.svg` |Use the URL to substitute the logo image in the About modal to be used in dark mode.
...
The https://github.com/hawtio/hawtio-react/tree/main/packages/hawtio/src/plugins/jmx[JMX] plugin is customisable via the `jmx` configuration in `hawtconfig.json`.
...
- https://github.com/hawtio/hawtio/tree/main/examples/quarkus
...
- https://github.com/hawtio/hawtio/tree/main/examples/springboot-authentication
...
configManager.configure(config => {
  ...
  config.branding = {
    ...
    appLogoDarkModeUrl: '/sample-plugin/branding/Logo-RedHat-A-DarkMode-RGB.png',
