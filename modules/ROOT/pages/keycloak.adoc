...
You can log in to the Keycloak admin console http://localhost:18080/admin/ with user `admin` / password `admin`. Import https://raw.githubusercontent.com/hawtio/hawtio/main/examples/keycloak-integration/hawtio-demo-realm.json[hawtio-demo-realm.json] into Keycloak. To do so, click `Create Realm` button and then import `hawtio-demo-realm.json`. It will create `hawtio-demo` realm.
...
See https://github.com/hawtio/hawtio/tree/main/examples/quarkus-keycloak[quarkus-keycloak example] for a working example.
...
See https://github.com/hawtio/hawtio/tree/main/examples/springboot-keycloak[springboot-keycloak example] for a working example.
