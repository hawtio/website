...
Hawtio 5 is a major version with the primary objective of upgrading to PatternFly v6.

Hawtio relies on two core technology stacks: React and PatternFly. Of these, PatternFly undergoes major upgrades every few years, and Hawtio must keep pace with these upgrades to maintain a modern look and feel.

While this UI upgrade does not break the compatibility of Hawtio's core functionality, it does introduce breaking changes to custom plugins and the external ecosystem. Because these plugins must migrate to the new PatternFly version, such upgrades are treated as breaking changes under semantic versioning and are therefore delivered as a major version release.
